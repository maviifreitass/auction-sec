...
@Named
@ApplicationScoped
public class PostgresWrapperClient {
    ...
    public Boolean openPostgresConnection() {
        try {
            String url = "jdbc:postgresql://localhost:5432/auction-client";
            connection = DriverManager.getConnection(url, "admin-nutrition", "admin");
        } catch (SQLRecoverableException ex) {
            Logger.getLogger(PostgresWrapperClient.class.getName()).log(Level.SEVERE, "Network connection error!", ex);
            return false;
        } catch (SQLException ex) {
            Logger.getLogger(PostgresWrapperClient.class.getName()).log(Level.SEVERE, "Database access error!", ex);
            return false;
        }
        return true;
    }
    ...
    public void closeConnection() {
        try {
            ...
            Logger.getLogger(PostgresWrapperClient.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
}
